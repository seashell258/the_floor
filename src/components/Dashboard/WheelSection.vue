  ...
  <section class="section">
    ...
    <div class="wheel-inner">
      <div class="wheel-controls">
        ...
        <button class="wheel-btn" @click="onShowRemoveDialog" :disabled="wheelPlayers.length === 0">移除陣亡者</button>
      </div>
      ...
      <div v-if="drawnPlayer" class="drawn-result">
        <p><strong>抽中：</strong>{{ drawnPlayer.name }}</p>
      </div>
    </div>

    <div v-if="showRemoveDialog" class="modal-overlay" @click="onHideRemoveDialog">
      <div class="modal-content" @click.stop>
        ...
        <div class="player-list-modal">
          <div v-for="player in wheelPlayers" :key="player.name" class="player-item">
            ...
            <button class="remove-btn" @click="onRemovePlayer(player.name)">移除</button>
          </div>
        </div>
        <div class="modal-actions">
          <button class="cancel-btn" @click="onHideRemoveDialog">取消</button>
        </div>
      </div>
    </div>
  </section>
